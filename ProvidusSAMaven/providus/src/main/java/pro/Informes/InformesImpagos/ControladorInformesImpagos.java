...
public class ControladorInformesImpagos extends ControladorInformes {
    private InterfazInformesImpagos ventanaInformes;
    private Informe informe;

    public ControladorInformesImpagos(InterfazInformesImpagos ventanaInformes, Main control) {
        super(control);
        this.ventanaInformes = ventanaInformes;
        this.informe = new Informe(ventanaInformes.getTitulo().getText(), "IMPAGOS");
        agregarManejadores();

    }

    public ControladorInformesImpagos(InterfazInformesImpagos ventanaInformes, Main control,Informe informe) {
        super(control);
        this.ventanaInformes = ventanaInformes;
    ...
    protected void confirmar() {
        ...
        try {
            ...
            if (resultado.next()) {
                ...
                Fila fila = new Fila(nombre, apellido, localidad, numeroPlan, numeroCuota, motivo);
    ...
    protected void guardar() {
        ...
        for (Fila fila : lista) {
            sql = String.format(Locale.US,
                    "INSERT IGNORE INTO CUOTA(id_plan,num_cuota,importe_cuota,estado) values(%d,%d,%f,\"%s\")",
            ...
            sql = String.format(Locale.US,
                    ...
                    ventanaInformes.getMotivo().getText());
            ConexionSQL.operacionIUD(sql);
        }
    }
    ...
    protected void recuperarInforme() {
        ...
        String sql = String.format(
                "SELECT nombre,apellido,localidad,di.id_plan,cu.num_cuota,observacion FROM DETALLE_INFORME di JOIN CUOTA cu ON di.id_plan=cu.id_plan AND di.num_cuota=cu.num_cuota JOIN PLAN pl ON di.id_plan=pl.id_plan JOIN CLIENTE cli ON cli.id_cliente=pl.id_cliente WHERE id_informe=%d;",
                idInforme);
        ...
        try {
            while (resultado.next()) {
                ...
                String motivo=resultado.getString(6);
